...
// enter haunted scene
let canvasEl = document.querySelector('.webgl');
...
if(btn) {
    btn.addEventListener("click", () => {
        ...
        howToUse.classList.remove("howtouse");
...
for(let i=0; i<50; i++){
    ...
    grave.position.set(x, grave.geometry.parameters.height/2, z);
...
const doorLightGeometry = new THREE.SphereBufferGeometry(0.09, 16, 16);
const doorLightMaterial = new THREE.MeshStandardMaterial({
    color: 0xff7d46
})
const doorLightModel = new THREE.Mesh(doorLightGeometry, doorLightMaterial);

doorLightModel.position.set(0, 2.3, 2);
house.add(doorLightModel);
...
const tick = () =>
{
    ...
    ghost3.position.y = Math.sin(elapsedTime * 3) + Math.sin(elapsedTime * 4.5);
